...
// Severity levels for osquery_embed_log_callback, mirroring osquery's own
// StatusLogSeverity (see osquery/core/plugins/logger.h): 0=INFO,
// 1=WARNING, 2=ERROR, 3=FATAL.
typedef void (*osquery_embed_log_callback)(int32_t severity,
                                            const char* filename,
                                            size_t filename_len,
                                            int32_t line,
                                            const char* message,
...
// Registers a callback that receives every osquery-internal status log
// message (INFO/WARNING/ERROR/FATAL) generated by the embedded runtime,
// instead of them going straight to stderr -- see osquery_embed_init's own
// doc comment for how this is wired into osquery's logger plugin registry.
// Call before osquery_embed_init to also capture log lines generated
// during startup itself; safe to call at any other time too (applies to
// every message logged after the call returns). Pass NULL to stop
// forwarding. The callback may be invoked from a thread other than the one
// that called osquery_embed_init/osquery_embed_query -- osquery's own
// logging can happen from any internal thread.
void osquery_embed_set_log_callback(osquery_embed_log_callback callback);
